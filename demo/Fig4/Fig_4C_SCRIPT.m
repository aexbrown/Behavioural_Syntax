% script to find n-grams that are used with different frequencies in
% different conditions.  All n-grams are tested and the false discovery
% rate is controlled across all comparisons.
%
% Reproduces Fig. 4C
...
for n = 1:3
    ...
    for ii = 1:numel(uniqueNames)-1
        ...
        for jj = ii+1:numel(uniqueNames)
            ...
            % test each sequence for a significant difference
    ...
    save(['Fig_4C_WORKSPACE_' num2str(n) '-gram.mat'])
end

% clear all previous calculations to be safe
clear all

% load p-values and correct for multiple comparisons across all tests
% performed for all n
pValsTotal = [];
for n = 1:3
    load(['Fig_4C_WORKSPACE_' num2str(n) '-gram.mat'], 'pValCell')
